...
import React from 'react';
 ...
 export default class TypeWriter extends React.Component{
    constructor(props){
        ...
        this.clock = false;
        this.counter=1;
        this.state = {flag:false, blinkIndex:0, stringList:quotes[random].split(""), wpm:0, colorList: new Array(quotes[random].length).fill(false)};
    ...
    render(){
        ...
        if(!this.state.flag){
            ...
            ele = (
                ...
                this.state.stringList.map((name, index) => {
                    if(name===" "){
                        return <span key={index} id={index} className={index===blinkIndex && this.state.colorList[index]?"blink_me color1" : this.state.colorList[index]?"color1":index===blinkIndex?"blink_me color4":"color3"}>␣</span>
                    }
                    else{
                        return <span key={index} id={index} className={index===blinkIndex && this.state.colorList[index]?"blink_me color1" : this.state.colorList[index]?"color1": index===blinkIndex?"blink_me color4":""} >{name}</span>
                    }
